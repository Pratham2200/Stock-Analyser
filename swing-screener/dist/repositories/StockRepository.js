...
class StockRepository extends BaseRepository_1.BaseRepository {
    constructor(pool) {
        super(pool, 'StockRepository');
    }
    async createScan(totalStocksScraped, stocksAnalyzed, stocksPassed, scanDurationSeconds) {
        const { text, values } = this.buildInsertQuery('scans', {
    ...
    async insertStocks(scanId, stocks) {
        if (stocks.length === 0) {
        ...
        return this.transaction(async (client) => {
            ...
            for (const stock of stocks) {
                const { text, values } = this.buildInsertQuery('stocks', {
                    scan_id: scanId,
                    symbol: stock.symbol,
                    name: stock.name
                }, ['id', 'symbol', 'name']);
    ...
    async insertStockAnalysis(stockId, scanId, analysisResult) {
        const { text, values } = this.buildInsertQuery('stock_analysis', {
            stock_id: stockId,
            scan_id: scanId,
    ...
    async insertSelectedStock(stockId, scanId, selectedStockData) {
        const { text, values } = this.buildInsertQuery('selected_stocks', {
            stock_id: stockId,
            scan_id: scanId,
    ...
    async getAllStocksWithAnalysis() {
        const text = `
      SELECT 
        st.id, st.symbol, st.name,
        sa.qualified, sa.fail_step, sa.fail_reason, sa.current_price,
    ...
    async getAnalysisStatistics(scanId) {
        let whereClause = '';
        let params = [];
        if (scanId) {
    ...
    async deleteOldScans(olderThanDays = 30) {
        const text = `
      DELETE FROM scans 
     ...
     */
    async getStocksFromTodayWithIds(scanId) {
        const text = `
     ...
     */
    async hasAnalysisForTodayStocks() {
        const text = `
      SELECT 
     ...
     */
    async getTodayStocksWithAnalysis() {
        const text = `
      SELECT 
