...
class StockDataService extends BaseService_1.BaseService {
    constructor() {
        ...
        this.MAX_RETRIES = 3;
        this.INITIAL_RETRY_DELAY_MS = 2000; // 2 seconds initial retry delay
     ...
     */
    async fetchDailyBars(symbol, days = 120, retryCount = 0) {
        try {
            ...
            const result = await yahoo_finance2_1.default.historical(yahooSymbol, queryOptions);
    ...
    async fetchCurrentPrice(symbol) {
        try {
            ...
            await this.rateLimiter.waitIfNeeded();
            const yahooSymbol = this.convertToYahooSymbol(symbol);
            const quote = await yahoo_finance2_1.default.quote(yahooSymbol);
