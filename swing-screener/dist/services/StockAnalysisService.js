...
exports.StockAnalysisService = void 0;
const BaseService_1 = require("./BaseService");
const SwingStrategyService_1 = require("./SwingStrategyService");
const technicalindicators_1 = require("technicalindicators");
class StockAnalysisService extends BaseService_1.BaseService {
    constructor() {
        super('StockAnalysisService');
        this.swingStrategyService = new SwingStrategyService_1.SwingStrategyService();
    }
    async analyzeStock(input) {
        ...
        try {
            ...
            if (input.dailyBars.length < 80) {
                return this.createRejectedResult('Insufficient data', 'Need at least 80 days of data for reliable analysis', startTime, input.dailyBars.length, input.intradayBars?.length || 0);
            }
            ...
            return this.createRejectedResult('Analysis error', error.message, startTime, input.dailyBars.length, input.intradayBars?.length || 0);
        }
    }
    async analyzeWithSwingStrategy(input) {
        try {
            this.logger.info(`🎯 Starting 4-Rule Swing Strategy Analysis for ${input.symbol}`);
            // Convert DailyBar format to SwingDailyBar format
            const swingDailyBars = input.dailyBars.map(bar => ({
            ...
            this.logger.stockAnalysis(input.symbol, swingResult.qualified, swingResult.score, swingResult.qualified ? undefined : swingResult.reason);
    ...
    convertSwingDetailsToAnalysisDetails(swingDetails) {
    ...
    async performAnalysis(dailyBars) {
    ...
    async checkConsolidationPhase(bars) {
        try {
            const closes = bars.map(bar => bar.close);
            const ema10 = technicalindicators_1.EMA.calculate({ values: closes, period: 10 });
            const ema20 = technicalindicators_1.EMA.calculate({ values: closes, period: 20 });
            if (ema10.length < 20) {
            ...
            return { pass: false, status: 'error', reason: error.message };
        }
    }
    async checkHigherLowStructure(bars) {
        try {
    ...
    calculateOverallScore(details) {
        let score = 0;
        if (details.consolidation.pass)
            score += 25;
        if (details.higherLow.pass)
            score += 25;
        if (details.volumePump.pass)
            score += 25;
        if (details.bearSqueeze.pass)
            score += 25;
        return score;
    }
    determineQualification(details, score) {
        return score >= 75 && details.consolidation.pass && details.higherLow.pass;
    }
    getFailureStep(details) {
        if (!details.consolidation.pass)
            return 1;
        if (!details.higherLow.pass)
            return 2;
    ...
    getFailureReason(details) {
        if (!details.consolidation.pass)
            return 'Consolidation phase not detected';
        if (!details.higherLow.pass)
            return 'Higher low structure not found';
        if (!details.volumePump.pass)
            return 'Volume pump not detected';
        if (!details.bearSqueeze.pass)
            return 'Bear squeeze pattern not found';
        return 'Unknown failure';
    }
    calculateOverallResult(consolidation, higherLow, volumePump, bearSqueeze) {
        const score = this.calculateOverallScore({ consolidation, higherLow, volumePump, bearSqueeze, overall: {} });
        let grade;
        if (score >= 90)
            grade = 'A';
        else if (score >= 75)
    ...
    createRejectedResult(reason, details, startTime, dailyCount, intradayCount) {
        return {
            qualified: false,
            score: 0,
            failedAt: 1,
            ...
            details: {
                ...
                higherLow: { pass: false, status: 'failed', reason, higherLowCount: 0, recentLows: [], trend: 'down', strength: 0 },
                volumePump: { pass: false, status: 'failed', reason, volumeRatio: 0, averageVolume: 0, currentVolume: 0, volumeTrend: 'decreasing' },
...
exports.StockAnalysisService = StockAnalysisService;
