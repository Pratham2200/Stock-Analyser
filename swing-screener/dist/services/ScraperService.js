...
class ScraperService extends BaseService_1.BaseService {
    constructor() {
        super('ScraperService');
        this.CHARTINK_URL = 'https://chartink.com/screener/copy-r2-bear-squeeze-setup-bss-13';
    }
    async scrapeAllStocks() {
        const startTime = Date.now();
        let browser = null;
        try {
            this.logger.info('🚀 Starting stock scraping process for LATEST stocks');
            this.logger.info(`📊 Source: ${this.CHARTINK_URL}`);
            ...
            try {
                ...
                this.logger.info(`🌐 Navigating to Chartink screener...`);
                await this.navigateToUrl(page, this.CHARTINK_URL);
                ...
                if (totalStocks > 0) {
                    const stocks = await this.extractAllStocks(page, totalStocks, seenSymbols);
                    allStocks.push(...stocks);
                    this.logger.success(`✅ Found ${stocks.length} unique stocks`);
            ...
            - The page failed to load properly`);
                ...
                this.logger.error(`❌ Error scraping from Chartink:`);
    ...
    async extractAllStocks(page, totalStocks, globalSeenSymbols) {
        const allStocks = [];
        const localSeenSymbols = new Set(); // For deduplication within this source
        let currentPage = 1;
        ...
        while (currentPage <= maxPages) {
            ...
            if (currentPage > 1 && pageStocks.length === previousPageStockCount) {
                const allSameStocks = pageStocks.every((stock) => localSeenSymbols.has(stock.symbol));
                if (allSameStocks) {
    ...
    async navigateToNextPage(page) {
        try {
            ...
            this.logger.error('Error navigating to next page:', error.message);
            return false;
        }
    }
}
