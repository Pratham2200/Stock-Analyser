// src/services/GroqAnalysisService.ts - AI Analysis using Groq (Llama)
// Fallback provider when Gemini hits rate limits
...
export class GroqAnalysisService extends BaseService {
    private client: Groq;
    private modelName: string;

    constructor(apiKey: string, model?: string) {
        super('GroqAnalysisService');

        this.client = new Groq({ apiKey });
        this.modelName = model || 'llama-3.3-70b-versatile';
        this.logger.info(`Groq fallback initialized with model: ${this.modelName}`);
    }

    /**
     * Analyze a stock using Groq as fallback provider
     */
    async analyzeStock(input: AIAnalysisInput): Promise<AIAnalysisOutput> {
        ...
        try {
            ...
            this.logger.info(`Groq Analysis complete for stock ${input.stockId}: ${parsed.decision} (${parsed.confidence}% confidence)`);
            ...
            this.logger.error(`Groq Analysis failed for stock ${input.stockId}:`, error);
            return { success: false, log, error: errorMessage };
        }
    }

    /**
     * Build prompt - identical to Gemini prompts for consistency
     */
...
REASONING: [2-3 sentences explaining your validation]`;
    ...
    /**
     * Parse AI response text - same logic as Gemini
     */
